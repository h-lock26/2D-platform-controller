...
enum {IDLE, WALK, JUMP, FALL, WALL_SLIDE, CROUCH}
...
@export var ACTION_DOWN := "ui_down" ## The input mapping for down
...
@export_range(0, 1, 0.01) var WALL_TIMER: float = 0.08
...
@onready var timer = $"../damage_zone_pit/Timer"
...
func manage_state() -> void:
	if Input.is_action_pressed("crouch"):
		...
		if can_wall_jump and ray_cast_2d_right.is_colliding() or ray_cast_2d_left.is_colliding():
			state = WALL_SLIDE
		else:
			state = FALL
...
func force_crouch():
	if is_on_floor() and ray_cast_2d.is_colliding():
		state == CROUCH
		jump_count = -1
...
func handle_gravity(delta: float) -> void:
	...
	if can_wall_jump and state == WALL_SLIDE and not jumping:
		velocity.y = clampf(velocity.y, 0.0, WALL_SLIDE_SPEED * 1.5)
...
func handle_jump(delta: float, move_direction: Vector2, jump_strength: float = 0.0, jump_pressed: bool = false, _jump_released: bool = false) -> void:
	if (jump_pressed or should_jump) and can_jump:
		...
		cancel_jump(delta)
	elif can_wall_jump and !is_on_floor() and is_on_wall_only():
		can_jump = true
		jumping = false
		wall_jump = true
		jump_count = 0
	...
	if (jump_pressed) && !is_on_floor() && !is_on_wall() && jump_count < max_jumps && state == JUMP:
		velocity.y == 0
		JUMP_FORCE *= .9
		apply_jump(move_direction)
		jump_count +=1
	...
	if (jump_pressed) && !is_on_floor() && !is_on_wall() && jump_count < max_jumps && state == FALL:
		velocity.y = 0
		JUMP_FORCE *= .8
		apply_jump(move_direction)
		jump_count +=1
...
func wall_time() -> void:
	await get_tree().create_timer(WALL_TIMER).timeout
	wall_jump = false
